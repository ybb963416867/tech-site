Android性能优化之APK瘦身详解(瘦73%)



![](images/4D9135959D4142259CEA3E49F5336AFD8503FA8555E340E4897B5AADACFA7A9D.png)
...
- 2. 使用[tinypng](https://tinypng.com/)等图片压缩工具对图片进行压缩。
...
![这里写图片描述](images/7E348B1DACD2491D9BCEBFA8D7D8467C.png)

APK包结构如下：

1. lib/：包含特定于处理器软件层的编译代码。该目录包含了每种平台的子目录，像armeabi，armeabi-v7a， arm64-v8a，x86，x86_64，和mips。大多数情况下我们可以只用一种armeabi-v7a，后面会讲到原因。

1. assets/：包含应用可以使用AssetManager对象检索的应用资源。

1. res/：包含未编译到的资源 resources.arsc,主要有图片资源文件。

1. META-INF/：包含CERT.SF和 CERT.RSA签名文件以及MANIFEST.MF 清单文件。

1. resources.arsc：包含已编译的资源。该文件包含res/values/ 文件夹所有配置中的XML内容。打包工具提取此XML内容，将其编译为二进制格式，并将内容归档。此内容包括语言字符串和样式，以及直接包含在resources.arsc文件中的内容路径 ，例如布局文件和图像。

1. classes.dex：包含以Dalvik / ART虚拟机可理解的DEX文件格式编译的类。

1. AndroidManifest.xml：包含核心Android清单文件。该文件列出应用程序的名称，版本，访问权限和引用的库文件。该文件使用Android的二进制XML格式。
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-xNDtH3zt-1571353784450)(http://upload-images.jianshu.io/upload_images/3965704-460ebedc384ceb72?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)]
...
![这里写图片描述](images/CCC19F8D935B490D9E13081C7B8CEAEC.png)

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-Xv3lhgYo-1571353784451)(http://upload-images.jianshu.io/upload_images/3965704-3ec00db43f5820e1?imageMogr2/auto-orient/strip)]
...
![这里写图片描述](images/2A7879829B3543F59F8FE8DB6993048D.png)
...
![这里写图片描述](images/D25558CF4D0749D0BD1BA4798AF88348.png)
...
[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-aczX7vG1-1571353784454)(http://upload-images.jianshu.io/upload_images/3965704-8d3d8965c8175236?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)]
...
![这里写图片描述](images/3DC35419F69741FEAF2F1A6620979FD0.png)
...
1. 避免使用枚举，这里特别去网上查了一下，具体可以参考下这篇文章Android 中的 Enum 到底占多少内存？该如何用？，得出的结论是，可能几十个枚举的内存占有量才相当一张图片这样子，优化效果也不会特别明显。当然，如果你是个追求极致的人，我不反对你用静态常量替代枚举。

1. 如果你的dex文件太大，检查是否引入了重复功能的第三方库（图片加载库，glide,picasso,fresco,image_loader，如果不是你一个人单独开发完成的很容易出现这种情况），尽量做到一个功能点一个库解决。
...
1. 尝试将andorid support库彻底踢出你的项目。

1. 尝试使用动态加载so库文件，插件化开发。

1. 将大资源文件放到服务端，启动后自动下载使用。
...
1. APK 瘦身记，如何实现高达 53% 的压缩效果

1. 使用APK Analyzer分析你的APK

1. 安卓app打包的时候还需要兼容armeabi么？

1. 百度百科webp

1. Android 中的 Enum 到底占多少内存？该如何用？
